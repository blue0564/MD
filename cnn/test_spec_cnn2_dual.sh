...
if [ $stage -le 1 ]; then
  ...
  do
    ...
    echo ${spec_opts}
    python cnn/make_cnn_egs2_dual.py -d -v ${spec_opts} --rttm-file=${rttmfile} $wavfile  $decdir/${filename}.npy $decdir/${filename}_with_sil.pos
 ...
 # ckpt=10000
  ...
  do
    ...
    do
      ...
      python cnn/predCNN_dual.py --checkpoint=${ckpt} --out-predlab=${labfile} ${datfile} ${posfile} ${dnnmdl} ${class_conf} ${logfile}
 ...
 # ckpt=10000
  ...
  do
    ...
    do
      ...
      python cnn/extract_CNN_conv_output_dual.py --checkpoint=${ckpt} ${datfile} ${posfile} ${dnnmdl} ${convdatafile} ${convposfile} ${logfile}
      echo "Plot t-SNE"
