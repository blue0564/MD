...
open(FILE, "<$ref_file") || die "praat2rttm.pl, Error! open input\n";
...
push(@list,$matrix[0][2*$_],$matrix[0][2*$_+1]) foreach(0...$classNum-1);	# fixed by hwh at 17.10.31.1517

#print $matrix[0][1]." ".$ind[1]."\n";
...
$cnt=0;
$mean=0;
while($cur<$maxVal[0] & $mean<$maxVal[0]+0.9999){
#while($cnt<1){
#print "cnt is ".$cnt."\n";
#print "==\n";
...
$dur=$a[1];
$class=$a[2];
$result=sprintf("SPEAKER $file_name[0] 1 %.3f %.3f <NA> <NA> $class <NA> <NA>\n",$begin_time,$dur);	# fixed by hwh at 17.10.31.1530
...
#print "max value is ".$maxVal[0]+0.9999."\n";
