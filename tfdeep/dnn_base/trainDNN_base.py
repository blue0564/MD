#!/usr/bin/env python
#
# copyright 2018.3. byeongYong, Jang
#
# This script is for training DNN model.
# If you want to change the model structure like the number of layer or node, etc, 
# you will modify the pretty much code.
# 
# modified code in 2013.03.23 by WoonHaeng, Heo
# add logging code for writing and saving log file
# 
...
# the following two line is modified by WoonHane, Heo in 2018.03.23
...
usage="%prog [options] <train-data-file> <train-label-file> <directory-for-save-model> <log-file>"
...
parser.add_option('--active-function', dest='actFunc', 
                   help='active function relu or sigmoid [default: %default]', 
                   default='relu', type='string');

(o,args) = parser.parse_args()
(trDataFile, trLabelFile, expdir, logFile) = args
...
hidNode6 = hidNode_map['hid6']
hidNode7 = hidNode_map['hid7']
### End parse options 
...
### Main script ###
mylogger.info('######### Configuration of DNN-model #########')
mylogger.info('# Dimension of input data = %d, # of classes = %d' %(o.inDim,o.numClass))
mylogger.info('# Layer size = %d layer, active function: %s',len(hidNode_map), o.actFunc)
mylogger.info('# Mini-batch size = %d, # of epoch = %d' %(miniBatch,nEpoch))
mylogger.info('# Learning rate = %f, probability of keeping in dropout = %0.1f' %(lr,o.kProb))
mylogger.info('LOG : train data size = %d, # of iterations = %d' %(trData.shape[0],totalBatch))
mylogger.info('LOG : validation data size is %d (%d%%) of %d training data' %(valInx,valRate,(oriTrData.shape[0]+1)))
...
[W1, b1, h1] = build_layer(o.inDim, hidNode1, x, o.actFunc) 
h1Drop = tf.nn.dropout(h1, keepProb)

[W2, b2, h2] = build_layer(hidNode1, hidNode2, h1Drop, o.actFunc) 
h2Drop = tf.nn.dropout(h2, keepProb)

[W3, b3, h3] = build_layer(hidNode2, hidNode3, h2Drop, o.actFunc) 
h3Drop = tf.nn.dropout(h3, keepProb)

[W4, b4, h4] = build_layer(hidNode3, hidNode4, h3Drop, o.actFunc) 
h4Drop = tf.nn.dropout(h4, keepProb)

[W5, b5, h5] = build_layer(hidNode4, hidNode5, h4Drop, o.actFunc)
h5Drop = tf.nn.dropout(h5, keepProb)

[W6, b6, h6] = build_layer(hidNode5, hidNode6, h5Drop, o.actFunc)
h6Drop = tf.nn.dropout(h6, keepProb)

[W7, b7, h7] = build_layer(hidNode6, hidNode7, h6Drop, o.actFunc)
h7Drop = tf.nn.dropout(h7, keepProb)

W_last = weight_variable([hidNode7, o.numClass],"W_last")
b_last = bias_variable([o.numClass],"b_last")
out_y = tf.add(tf.matmul(h7Drop,W_last,name="mm_last"),b_last,name="out_y")
...
while(epoch<nEpoch):
  ...
  if epoch%o.valEpoch==0: # print state of training for validation data
    ...
    file_handler.setFormatter(formatter)
    mylogger.addHandler(file_handler)
    mylogger.info('Training %d epoch : ce = %f, acc = %2.1f%% ' %(epoch,(val_ce/valData.shape[0]),(val_acc*100)))
