...
# train dual input CNN model using drama database
...
if [ $stage -le 0 ]; then
  ...
  do
    ...
    echo ${spec_opts}
    python cnn/make_cnn_egs2_dual.py -d -v ${spec_opts} --rttm-file=${rttmfile} $wavfile  $decdir/spec_data.npy $decdir/spec_data_with_sil.pos
...
if [ $stage -le 1 ]; then
  ...
  python cnn/trainCNN_dual.py --num-epoch 500 --minibatch 100 \
